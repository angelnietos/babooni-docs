# Isomorphic validation — F72-B1 close
...
| Zod / dual-schema kit | **Defer F73** — capacidad + BE↔shared DTO drift (`clients.dtos.ts` Nest-only); predicates + ADR 0012 remain SoT |
...
| F71-B1 close | Defer F72 con owner + blocker explícito (sin silencios) |
| F72-B1 close | Defer F73 con owner + blocker explícito (sin silencios) |

Do **not** leave as silent “listo”. Resume Zod kit via **F73**
with owner + ADR addendum; predicates stay FE runtime SoT until paridad.

Carry record: F68 → F69 → F70 → F71 → F72 deferred.
