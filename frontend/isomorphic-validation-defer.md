# Isomorphic validation — F71-B1 close
...
| Zod / dual-schema kit | **Defer F72** — capacidad + BE↔shared DTO drift (`clients.dtos.ts` Nest-only); predicates + ADR 0012 remain SoT |
...
Do **not** leave as silent “listo”. Resume Zod kit via **F72**
with owner + ADR addendum; predicates stay FE runtime SoT until paridad.

Carry record: F68 → F69 → F70 → F71 deferred.
